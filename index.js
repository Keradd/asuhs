...
const networkConfig = {
  name: 'Pharos Testnet',
  chainId: 688688,
  rpcUrl: 'https://api.zan.top/node/v1/pharos/testnet/b940e2b11db140ca9079cd18ba17508e',
  currencySymbol: 'PHRS',
};
...
const shuffleTasks = (tasks) => {
  const tasksExceptAddLP = tasks.filter(task => task !== 'addLP');
  for (let i = tasksExceptAddLP.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [tasksExceptAddLP[i], tasksExceptAddLP[j]] = [tasksExceptAddLP[j], tasksExceptAddLP[i]];
  }
  return [...tasksExceptAddLP, 'addLP'];
};

const delayHuman = async () => {
  const delay = Math.floor(Math.random() * 4000) + 2000; // 2-6 detik
  logger.info(`Waiting ${delay / 1000}s before next action...`);
...
const main = async () => {
  ...
  while (true) {
    for (const privateKey of privateKeys) {
      ...
      const numTransfers = Math.floor(Math.random() * 5) + 6; // 6–10
      const numWraps = Math.floor(Math.random() * 5) + 6;
      const numSwaps = Math.floor(Math.random() * 5) + 6;
      const numLPs = Math.floor(Math.random() * 5) + 6;

      const tasks = shuffleTasks(['transfer', 'wrap', 'swap', 'addLP']);
      ...
      const delayBetweenWallets = Math.floor(Math.random() * 10000) + 5000; // 5–15 detik
